...
//if ($conteo == 2) { header ("Location: planillas_.php?id=$id_zonas"); }
if ($conteo == 2) { header ("Location: planillas_.php?id=$id_zonas"); }
...
$nombre_zona = 	$row['zona'];
	...
	if ($conteo != 4) {
					 ...
					 $queryqa ="SELECT Planilla FROM fi_conteos where Planilla = $i and Bodega = $num_tienda and estado =1 and conteo = $conteo and Usuario = '$usuario'"; 
					 
$resultqa = mysqli_query($connect, $queryqa); 
if (mysqli_num_rows($resultqa)> 0){?>
	<a href="inv.php?id_planilla=<?=$i.$aux_id_get1.$id_zonas?>"><span class="button"><p><?=$i?></p></span></a>
<?php 
	}
...
<?php 
		...
		for ($i=$rango_inicial; $i<=$rango_final; $i++)	{
					 $queryqa ="SELECT Planilla FROM fi_conteos where Planilla = $i and Bodega = $num_tienda and estado =2 and conteo = $conteo and usuario = '$usuario'"; 
...
?>
	<a href="inv.php?id_planilla=<?=$planillaqw.$aux_id_get1.$id_zonas?>"><span class="button"><p><?=$planillaqw?></p></span></a>
<?php 	
}	
...
for ($i=$rango_inicial; $i<=$rango_final; $i++)	{
$queryq ="SELECT Planilla FROM fi_conteos where Planilla = $i and Bodega = $num_tienda and conteo = $conteo"; 
