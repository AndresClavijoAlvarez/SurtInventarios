...
$id_zonas =$_GET["id"];
 ...
 if ($conteo == 3) { $conteo = 4; $c_aux=1;
 
	$resa1 = $mysqli->query("UPDATE fi_conteos SET Conteo = 4  WHERE Conteo = 1 and zona = $id_zonas and Bodega = $num_tienda and Estado = 2"); 
				   }			   
...
.navbar > li:hover .icon20.login {background-position:-100px -20px;}
	...
	*/?>
           ...
           <table width="70%" border="10" align="center">
             <tbody>
               <tr>
                 <th colspan="2" scope="col" align="center"><div align="center">Mis planillas</div></th>
               </tr>
               <tr class="btn-danger">
...
$nombre_zona = 	$row['zona'];
	...
	if ($conteo != 4) {
					 ...
					 $queryqa ="SELECT Planilla FROM fi_conteos where Planilla = $i and Bodega = $num_tienda and estado =1 and conteo = $conteo"; 
					 
$resultqa = mysqli_query($connect, $queryqa); 
if (mysqli_num_rows($resultqa)> 0){?>
	<a href="inv_manual.php?id_planilla=<?=$i.$aux_id_get1.$id_zonas?>"><span class="button"><p><?=$i?></p></span></a>
<?php 
	}
...
$nombre_zona = 	$row['zona'];
	if ($c_aux == 1) {
					 ...
					 for ($i=$rango_inicial; $i<=$rango_final; $i++)	{
					 $queryqa ="SELECT Planilla FROM fi_conteos where Planilla = $i and Bodega = $num_tienda and estado =2 and conteo = $c_aux"; 
					 
$resultqa = mysqli_query($connect, $queryqa); 
if (mysqli_num_rows($resultqa)> 0){?>
	<span class="buttondd"><p><?=$i?></p></span>
<?php 
	}
					 }
	...
	if ($conteo == 2) {
	for ($i=$rango_inicial; $i<=$rango_final; $i++)	{
					 $queryqa ="SELECT Planilla FROM fi_conteos_manual where Planilla = $i and Bodega = $num_tienda and conteo =2"; 
					 
$resultqa = mysqli_query($connect, $queryqa); 
if (mysqli_num_rows($resultqa)> 0){?>
	<span class="button"><p><?=$i?></p></span>
<?php 
	}
	}
	}
...
FROM
fi_conteos AS FI,
fi_conteos_manual as TC
WHERE
FI.Planilla = $i AND
FI.Bodega = $num_tienda AND
FI.Estado = 2 AND
FI.Conteo = $conteo and 
TC.Planilla not in FI.Planilla"; 
...
<?php 
	...
	$queryqw ="SELECT * FROM planilla_c3 where zona = $nombre_zona and tienda = $num_tienda"; $resultqw = mysqli_query($connect, $queryqw); 
	
while($rowqw = mysqli_fetch_array($resultqw))  
	  {
$planillaqw = $rowqw['planilla'];
///////////////////////////////////////	
$querjp ="SELECT * FROM fi_conteos where Planilla = $planillaqw and Bodega = $num_tienda and conteo = 1 and estado = 2"; 
$resultjp = mysqli_query($connect, $querjp); 
while($rowjp = mysqli_fetch_array($resultjp))  {
$jp1 = $rowjp['Planilla'];

}	
////////////////////////////////////////////////////////
$queryqw2 ="SELECT * FROM fi_conteos where Planilla = $planillaqw and Bodega = $num_tienda and conteo = 1 and estado = 1"; 
$resultqw2 = mysqli_query($connect, $queryqw1); 
while($rowqw2 = mysqli_fetch_array($resultqw2))  {
$planillaqw2 = $rowqw2['planilla'];
}	
if (!empty($planillaqw2)) {
?>
	<a href="inv_manual.php?id_planilla=<?=$planillaqw.$aux_id_get1.$id_zonas?>"><span class="button"><p><?=$planillaqw?></p></span></a>
<?php 	
}	

if (empty($planillaqw2) && (empty($jp1))) {
?>
	<a href="inv_manual.php?id_planilla=<?=$planillaqw.$aux_id_get1.$id_zonas?>"><span class="button"><p><?=$planillaqw?></p></span></a>
<?php 
}
...
for ($i=$rango_inicial; $i<=$rango_final; $i++)	{
$queryq ="SELECT Planilla FROM fi_conteos_manual where Planilla = $i and Bodega = $num_tienda and conteo = $conteo"; 
