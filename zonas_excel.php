...
while($fila2 = mysql_fetch_assoc($resultado_zonas_p)){ 
	...
	$vv = ((mysql_num_rows($resultado_zonas)/$registros2)*100);	
 $vv = number_format($vv, 2, ',', '.');// 1234.57												  
echo '<font color=red><b>'.$vv." %".'</b></font>';
  ?></td>
...
$vble = ((mysql_num_rows($resultado_zonas)/$registros2)*100);
 $vble = number_format($vble, 2, ',', '.');// 1234.57												  
												  
echo '<font color=red><b>'.$vble." %".'</b></font>';
  ?></td>
<td><?=$registros2-mysql_num_rows($resultado_zonas)?></td>
